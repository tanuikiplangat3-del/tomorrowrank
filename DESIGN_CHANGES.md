...
## Update 57 — Global DataForSEO kill switch, with real fallbacks already in place for most of what it does

You're out of DataForSEO units. Rather than touching dozens of call sites, added ONE global switch that cascades everywhere, because every DataForSEO call site in this codebase already gated on `dataForSeoConfigured()` — turning that off in one place turns it off everywhere, and (usefully) most call sites already had a real fallback built, not just a blank gap:

1. **`dataForSeoConfigured()` now checks a `DATAFORSEO_DISABLED` env var first.** Set `DATAFORSEO_DISABLED=true` on the task definition (new revision needed — this is an env var change, not just a redeploy) and every DataForSEO call across the entire app stops immediately: AI visibility, SERP snapshot, ads volume, business profile, Bing/Yahoo/Images/Maps, On-Page cross-check — all of it, from this one flag.
2. **AI visibility (the single biggest consumer — ~20 calls/audit) already had a working Claude-only fallback built in** (`runWithClaudeOnly()` in `lib/ai-visibility/engine.ts`) that nobody had pointed out was already there: the dispatch logic already checked `dataForSeoConfigured()` before using the DataForSEO path. With the kill switch on, every audit automatically uses Claude (with real web search) asking the same buyer-intent prompts instead — real share-of-voice, sentiment, and insights, just sourced from Claude directly rather than polling ChatGPT/Gemini/Claude/Perplexity separately through DataForSEO. The new AI Responses dashboard (platform-by-platform mention counts, added in Update 47) is DataForSEO-specific and won't render in this mode — it disappears cleanly rather than showing fabricated numbers.
3. **SERP Snapshot already had a ScrapingBee fallback built in** (Update 51) for exactly this situation — with DataForSEO off, it now triggers on every audit automatically, no code change needed. Real position + People Also Ask; no featured-snippet/knowledge-panel detection (not available via ScrapingBee's schema, stays honestly blank rather than guessed).
4. **Real Google Ads search volume → Ahrefs' own keyword-volume figure**, newly wired as a fallback: when DataForSEO's Ads volume isn't available, the SERP Snapshot now shows Ahrefs' modelled volume for that keyword instead of leaving the field blank, if we already have it from the keyword data fetched earlier in the same audit.
