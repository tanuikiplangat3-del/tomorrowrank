...
## Update 43 — Diagnostics for "no real data" (NO demo data exists)

Confirmed via full-codebase scan: there is NO demo/mock/fake/hardcoded/sample data feeding results. Scores + issues come from the real crawl + real APIs. "No real data displayed" is a data-flow bug, most likely the crawl returning 0 pages (blocked) or throwing.
Root-cause aid: the crawl try/catch was SILENTLY swallowing errors. Now it logs. Added [audit] diagnostics:
- main page: status, protected, htmlLen, title (+ error if nearly empty).
- crawl done: proxy mode, protected, discovered, crawled, pages, issues, score, checked/notChecked (+ error if 0 pages, + the real error if the crawl throws, + warn if crawl skipped for low budget).
- ai-visibility: engine, aiOverview.present, prompt/answer count, score, brand mentions.
Deploy, run ONE audit, then `aws logs tail /ecs/ranktomorrow-task --since 10m` and read the [audit] lines to pinpoint exactly where data is lost.

---

## Update 44 — Cost control: DataForSEO now OPT-IN (Claude-only by default), no credits spent

User: DataForSEO using too many credits. Ahrefs Brand Radar exists on the account but is a monthly-tracked, per-brand database (empty for arbitrary visitor URLs), so it is NOT a live per-URL replacement — confirmed via MCP.
Change: runAiVisibility now runs DataForSEO ONLY when USE_DATAFORSEO_AI === "true" (was: on unless "false"). Default is the FREE Claude-only AI-visibility path, which works for any URL and spends ZERO DataForSEO credits. Logs which engine ran ([ai-visibility] engine=claude-only / dataforseo). DataForSEO code kept intact behind the flag so it can be re-enabled anytime by setting the env var.
No env change needed to stop the bleed — the default is now off. To re-enable live AI-engine data later: set USE_DATAFORSEO_AI=true on the task def.
