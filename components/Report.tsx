    ...
    <div className="relative z-10 mx-auto max-w-6xl px-4 py-10">
      ...
      <header className="glass-card p-6">
        <h1 className="font-display text-2xl font-extrabold text-white">
          SEO Audit for <span className="text-growLight">{prettyHost(report.meta.finalUrl)}</span>
        </h1>
        ...
        <p className="mt-2 text-xs text-white/70">
          Target: <span className="font-semibold text-growLight">{report.meta.country}</span> ·{" "}
          <span className="font-semibold text-growLight">{report.meta.language}</span>
      ...
      <section className="mt-6 glass-card p-6">
        <h2 className="font-display text-xl font-bold text-white">Audit Results</h2>
        <div className="mt-6 grid items-center gap-8 md:grid-cols-2">
          <div className="flex flex-col items-center">
            ...
            <p className="mt-3 font-display text-lg font-bold text-white">{report.overall.summary}</p>
            <span className="mt-2 rounded-md bg-red-500/15 px-3 py-1 text-sm font-semibold text-red-300">
      ...
      {/* Issues found */}
      <section className="mt-6 glass-card p-6">
        <h2 className="font-display text-xl font-bold text-white">Issues Found</h2>
      ...
      <section className="mt-6 glass-card p-6">
        <h2 className="font-display text-xl font-bold text-white">Recommendations</h2>
      ...
      {/* CTA */}
      <section className="mt-10 rounded-xl2 border border-white/15 bg-black/40 p-8 text-center backdrop-blur-sm">
        <h2 className="font-display text-2xl font-extrabold text-white">
          Ready to <span className="text-growLight">fix these?</span>
        </h2>
        <p className="mt-2 text-white/70">Welcome Tomorrow can implement every recommendation in this report.</p>
        <a href="https://welcometomorrow.io/contact"
          className="mt-5 inline-block rounded-lg bg-grow px-7 py-3 font-bold text-white transition hover:bg-growDeep">
          Let&apos;s do this →
        </a>
      </section>
    </div>
  ...
  return (
    <div className="flex items-start gap-3 py-3.5">
      ...
      <div className="flex-1">
        ...
        {check.detail && <p className="mt-0.5 text-sm text-white/70">{check.detail}</p>}
        {check.recommendation && (
          <p className="mt-0.5 text-sm text-growLight">→ {check.recommendation}</p>
        )}
      </div>
      {check.value != null && (
        <span className="max-w-[180px] truncate text-right text-xs text-white/70">{String(check.value)}</span>
      )}
    </div>
  );
}
  ...
  return (
    <section className="mt-6 grid gap-5 lg:grid-cols-2">
      ...
      <div className="glass-card p-6">
        <h3 className="font-display text-lg font-bold text-white">Top Organic Keywords</h3>
        <p className="mt-1 text-sm text-white/70">
          Est. traffic from search: {keywords.trafficFromSearch ?? "—"}
        </p>
        <table className="mt-3 w-full text-sm">
          <thead className="text-left text-xs uppercase tracking-wide text-white/70">
            <tr><th className="py-1">Keyword</th><th>Pos.</th><th>Volume</th></tr>
          </thead>
          <tbody>
            {keywords.organic.slice(0, 10).map((k, i) => (
              <tr key={i} className="border-t border-white/10">
                <td className="py-2 pr-2 text-white">{k.keyword}</td>
                <td className="font-semibold text-growLight">{k.position}</td>
                <td className="text-white/70">{k.searchVolume ?? "—"}</td>
              </tr>
            ))}
            {keywords.organic.length === 0 && (
              <tr><td colSpan={3} className="py-4 text-white/70">No ranking keywords found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {/* Backlinks */}
      <div className="glass-card p-6">
        <h3 className="font-display text-lg font-bold text-white">Backlink Profile</h3>
        ...
        <h4 className="mt-5 text-sm font-bold text-white">Top Backlinks</h4>
        <ul className="mt-2 space-y-1.5 text-sm">
          {backlinks.top.slice(0, 6).map((b, i) => (
            <li key={i} className="flex justify-between gap-2">
              <span className="truncate text-white/70">{b.sourceDomain}</span>
              <span className="font-semibold text-growLight">DR {b.domainAuthority ?? "—"}</span>
            </li>
          ))}
          {backlinks.top.length === 0 && <li className="text-white/70">No backlinks found.</li>}
        </ul>
      </div>

      {/* Performance */}
      <div className="glass-card p-6 lg:col-span-2">
        <h3 className="font-display text-lg font-bold text-white">PageSpeed & Core Web Vitals</h3>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {(["mobile", "desktop"] as const).map((s) => {
            const p = performance[s];
            return (
              <div key={s} className="rounded-lg glass-soft p-4">
                <p className="font-display font-bold capitalize text-white">{s}</p>
