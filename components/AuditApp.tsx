...
export function AuditApp() {
  ...
  return (
    <div className="relative z-10 mx-auto max-w-3xl px-4 py-16">
      {/* Heading — white, no underline (screenshot 2 theme) */}
      ...
      {phase === "input" || phase === "error" ? (
        <div className="mt-10">
          <div className="flex flex-col gap-3 rounded-xl2 border border-white/20 bg-white/[0.06] p-2 backdrop-blur-sm sm:flex-row sm:items-center sm:p-1.5">
            <input
              value={url}
              onChange={(e) => setUrl(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && start()}
              placeholder="Example.com"
              className="flex-1 rounded-lg bg-transparent px-4 py-3 text-lg text-white outline-none placeholder:text-white/40"
            />
            <button
              onClick={start}
              className="rounded-lg bg-grow px-8 py-3 text-lg font-bold text-white shadow-lg shadow-grow/20 transition hover:bg-growDeep"
            >
              Audit
            </button>
          </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            <div>
              ...
              <input value={keyword} onChange={(e) => setKeyword(e.target.value)}
                ...
                className="w-full rounded-lg border border-white/20 bg-white/[0.06] px-3 py-2.5 text-sm text-white outline-none backdrop-blur-sm placeholder:text-white/40 focus:border-grow" />
            </div>
          </div>
...
function Processing({ job, url, country, language }: {
  ...
  return (
    <div className="mt-14 flex flex-col items-center text-center">
      <div className="spinner h-16 w-16 rounded-full border-4 border-white/15 border-t-grow" />
      <div className="mt-8 w-full max-w-md">
        <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
          <div className="h-2 rounded-full bg-grow transition-all duration-500"
      ...
      <p className="mt-6 max-w-md text-white/70">
        ...
        <span className="font-bold text-white">{prettyHost(url)}, {country}, {language}.</span>
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="w-full rounded-lg border border-white/20 bg-white/[0.06] px-3 py-2.5 text-sm text-white outline-none backdrop-blur-sm focus:border-grow [&>option]:text-ink">
