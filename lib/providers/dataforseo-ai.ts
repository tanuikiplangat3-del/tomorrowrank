...
export function dataForSeoConfigured(): boolean {
  // Global kill switch — set DATAFORSEO_DISABLED=true on the task definition
  // to stop ALL DataForSEO usage instantly across the whole app (AI
  // visibility, SERP snapshot, ads volume, business profile, Bing/Yahoo/
  // Images/Maps, On-Page cross-check) without touching any other code —
  // every one of those call sites already gates on this same function, and
  // each already has a real, working fallback/graceful-skip for when it
  // returns false (see runWithClaudeOnly() in the AI-visibility engine, and
  // the ScrapingBee SERP fallback in the orchestrator).
  if (process.env.DATAFORSEO_DISABLED === "true") return false;
